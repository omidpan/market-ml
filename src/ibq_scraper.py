...
--------
Batch YAML:
    python ibq_scraper.py --config scraper-config-avgo.yml
...
# ---------------------------------------------------------------------------
# The scraper lives under <project>/src, so the project root itself contains
# no machine-specific absolute path and is safe to keep in Git.
...
def load_env_file(path: Path) -> None:
    ...
    Existing shell environment variables win over values from .env.
    Supported lines:
        KEY=value
        KEY="value"
        KEY='value'
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        # Environment variables explicitly supplied by the shell have
        # precedence over .env.
...
def require_env_path(name: str) -> Path:
    ...
    if raw is None or not raw.strip():
        raise RuntimeError(
            f"{name} is not defined. Add it to {ENV_FILE}."
        )
...
# Load the existing IB application environment only after the project .env has
# populated os.environ, so IB host/port variables can also live in the same
# project-root .env file if APPENV reads them from the environment.
...
def resolve_config_path(path: Path) -> Path:
    """
    Resolve scraper/config metadata paths.

    Absolute paths are accepted for CLI convenience.
    Relative paths are resolved under CONFIG_PATH from .env.
    """
...
def load_jobs(config_path: Path) -> RuntimeConfig:
    ...
    snapshot_root = resolve_data_path(
        config.get("snapshot_root"),
        default_relative="raw/bootstrap",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: $DATA_PATH/raw/bootstrap"
        ),
    )
...
def runtime_from_args(args: argparse.Namespace) -> RuntimeConfig:
    ...
    if not args.symbol:
        default_config = (
            CONFIG_PATH
            / "scraper-config-semiconductor.yml"
        )
