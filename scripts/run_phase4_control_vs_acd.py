...
# Default roots — unchanged local-run behavior. v1 remains frozen historical
# provenance; it is never regenerated by the active default path below.
...
# v2 (active/default) roots — TEST-sealed.
DEFAULT_MODEL_MATRIX_ROOT_V2 = phase3.DEFAULT_MODEL_MATRIX_ROOT_V2
DEFAULT_LABEL_POLICY_ROOT_V2 = REPO_ROOT / "data/parquet/label_policy_phase4_trainval_v2"
DEFAULT_LABEL_POLICY_REPORTS_ROOT_V2 = REPO_ROOT / "reports/label_policy_phase4_v2"
DEFAULT_MODELS_ROOT_V2 = REPO_ROOT / "data/models_phase4_trainval_v2"
DEFAULT_TRAINING_REPORTS_ROOT_V2 = REPO_ROOT / "reports/training_phase4_v2"
DEFAULT_GENERATED_CONFIG_DIR_V2 = REPO_ROOT / "config" / "generated"

FROZEN_TRAINVAL_TOTAL = phase3.FROZEN_TRAIN_COUNT + phase3.FROZEN_VALIDATION_COUNT
...
# v2 (active/default) run identifiers — distinct from the v1 names above so
# a run-id can never be ambiguous about which artifact contract it implies.
CONTROL_RUN_ID_V2 = "nvda_core_v1_phase4_v2_trainval"
ACD_RUN_ID_V2 = "nvda_core_v1_acd_v1_phase4_v2_trainval"
...
def generate_variant_configs(
    ...
    *,
    ...
    v2 (default) writes distinctly-named config files
    (pipeline_phase4_control_v2.yaml / _acd_v2.yaml) so a v1 config can
    never be silently reused for a v2 run, or vice versa. Config *content*
    (feature sets, frozen training keys) is identical either way — only the
    filename differs."""
    ...
    suffix = "" if legacy_v1 else "_v2"
    control_config_path = generated_config_dir / f"pipeline_phase4_control{suffix}.yaml"
    acd_config_path = generated_config_dir / f"pipeline_phase4_acd{suffix}.yaml"
...
def run_train(
    ...
    *,
    ...
    if legacy_v1:
        ...
        run_id = CONTROL_RUN_ID_V2 if variant == "control" else ACD_RUN_ID_V2
...
FROZEN_POLICY_SIGNATURE = (
    "cf167954f3b200118fb80fd50e680b4522cc3347cf08754a594f213945df6d9f"
)


def run_validation_v2(model_matrix_root: Path, label_policy_root: Path) -> bool:
    """
    v2 counterpart to run_validation(): CONTROL/ACD trainer-facing sequences
    and the shared label artifact all contain TRAIN+VALIDATION only (zero
    TEST rows), exactly 878,828/189,428 respectively, and policy_signature
    is byte-identical to the frozen value.
    """
    checks: list = []
    ...
    print("\nPhase-4 pre-training validation (v2, TEST-sealed):")
    ...
    check(
        "1. CONTROL/ACD sample counts match the frozen TRAINVAL total",
        len(control_seq) == len(acd_seq) == FROZEN_TRAINVAL_TOTAL,
        f"control={len(control_seq)} acd={len(acd_seq)} expected={FROZEN_TRAINVAL_TOTAL}",
    )

    control_split_counts = control_seq["split"].value_counts().to_dict()
    acd_split_counts = acd_seq["split"].value_counts().to_dict()
    check(
        "1b. CONTROL TRAIN/VALIDATION counts match frozen values",
        control_split_counts.get("train", 0) == phase3.FROZEN_TRAIN_COUNT
        and control_split_counts.get("validation", 0) == phase3.FROZEN_VALIDATION_COUNT,
        f"actual={control_split_counts}",
    )
    check(
        "1c. ACD TRAIN/VALIDATION counts match frozen values",
        acd_split_counts.get("train", 0) == phase3.FROZEN_TRAIN_COUNT
        and acd_split_counts.get("validation", 0) == phase3.FROZEN_VALIDATION_COUNT,
        f"actual={acd_split_counts}",
    )
    check(
        "1d. zero TEST rows in CONTROL/ACD trainer-facing sequences",
        int((control_seq["split"] == "test").sum()) == 0
        and int((acd_seq["split"] == "test").sum()) == 0,
    )

    labels = _load_label_policy_frame(label_policy_root)
    check(
        "2. label rows align to the shared TRAINVAL sequence universe",
        len(labels) == len(control_seq),
        f"labels={len(labels)} sequences={len(control_seq)}",
    )
    label_split_counts = labels["split"].value_counts().to_dict()
    check(
        "2b. label TRAIN/VALIDATION counts match frozen values, zero TEST",
        label_split_counts.get("train", 0) == phase3.FROZEN_TRAIN_COUNT
        and label_split_counts.get("validation", 0) == phase3.FROZEN_VALIDATION_COUNT
        and label_split_counts.get("test", 0) == 0,
        f"actual={label_split_counts}",
    )
    check(
        "3. policy name/signature is single-valued and matches the frozen value",
        labels["policy_name"].nunique() == 1
        and labels["policy_name"].iloc[0] == POLICY_NAME
        and labels["policy_signature"].nunique() == 1
        and labels["policy_signature"].iloc[0] == FROZEN_POLICY_SIGNATURE,
        f"actual_signature={labels['policy_signature'].iloc[0] if len(labels) else None}",
    )
    check(
        "4. no pre-2020-01-03 samples in labels",
        pd.to_datetime(labels["trading_date"]).min().date() >= COMMON_MODELING_START,
    ...
    check(
        "5. every CONTROL TRAINVAL sequence has a matching label row",
        aligned["label_direction"].notna().all(),
        f"unmatched={int(aligned['label_direction'].isna().sum())}",
    )
    check(
    ...
    check(
        "7b. CONTROL TRAIN keys == ACD TRAIN keys",
        set(
            zip(
                control_seq.loc[control_seq["split"] == "train", "source_id"],
                control_seq.loc[control_seq["split"] == "train", "prediction_time"],
            )
        )
        == set(
            zip(
                acd_seq.loc[acd_seq["split"] == "train", "source_id"],
                acd_seq.loc[acd_seq["split"] == "train", "prediction_time"],
            )
        ),
    )
    check(
        "7c. CONTROL VALIDATION keys == ACD VALIDATION keys",
        set(
            zip(
                control_seq.loc[control_seq["split"] == "validation", "source_id"],
                control_seq.loc[control_seq["split"] == "validation", "prediction_time"],
            )
        )
        == set(
            zip(
                acd_seq.loc[acd_seq["split"] == "validation", "source_id"],
                acd_seq.loc[acd_seq["split"] == "validation", "prediction_time"],
            )
        ),
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Explicit opt-in to the frozen v1 build path: consumes the "
            "full-value, all-splits Phase-3 v1 model_matrix and writes the "
            "full-value, all-splits label_policy_phase4_common_v1 "
            "artifact. Without this flag, the v2 path runs by default: "
            "TRAIN+VALIDATION-only trainer-facing artifacts, zero TEST "
            "rows anywhere."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument("--model-matrix-root", type=Path, default=None,
                         help=(
                             "Phase-3 output (read-only input to Phase 4). Defaults to "
                             "the v2 trainval root, or the v1 all-splits root if "
                             "--legacy-v1 is set."
                         ))
    parser.add_argument("--encoded-features-root", type=Path, default=DEFAULT_ENCODED_FEATURES_ROOT,
                         help="Phase-3 output (read-only input to Phase 4; unaffected by v1/v2).")
    parser.add_argument("--label-policy-root", type=Path, default=None)
    parser.add_argument("--label-policy-reports-root", type=Path, default=None)
    parser.add_argument("--models-root", type=Path, default=None)
    parser.add_argument("--training-reports-root", type=Path, default=None)
    ...
    model_matrix_root = args.model_matrix_root or (
        DEFAULT_MODEL_MATRIX_ROOT if legacy_v1 else DEFAULT_MODEL_MATRIX_ROOT_V2
    )
    label_policy_root = args.label_policy_root or (
        DEFAULT_LABEL_POLICY_ROOT if legacy_v1 else DEFAULT_LABEL_POLICY_ROOT_V2
    )
    label_policy_reports_root = args.label_policy_reports_root or (
        DEFAULT_LABEL_POLICY_REPORTS_ROOT if legacy_v1 else DEFAULT_LABEL_POLICY_REPORTS_ROOT_V2
    )
    models_root = args.models_root or (
        DEFAULT_MODELS_ROOT if legacy_v1 else DEFAULT_MODELS_ROOT_V2
    )
    training_reports_root = args.training_reports_root or (
        DEFAULT_TRAINING_REPORTS_ROOT if legacy_v1 else DEFAULT_TRAINING_REPORTS_ROOT_V2
    )

    config_suffix = "" if legacy_v1 else "_v2"
    control_cfg = args.generated_config_dir / f"pipeline_phase4_control{config_suffix}.yaml"
    acd_cfg = args.generated_config_dir / f"pipeline_phase4_acd{config_suffix}.yaml"

    if args.mode in ("labels", "all"):
        control_cfg, acd_cfg = generate_variant_configs(
            args.encoded_features_root, args.generated_config_dir, legacy_v1=legacy_v1,
        )
        build_labels(
            model_matrix_root, label_policy_root, label_policy_reports_root,
            legacy_v1=legacy_v1,
        )
    ...
    if args.mode in ("preflight", "all"):
        ...
        run_preflight(model_matrix_root, label_policy_root, control_cfg, acd_cfg)

    if args.mode == "train-control":
        run_train("control", args.confirm_training, control_cfg, acd_cfg,
                   model_matrix_root, label_policy_root,
                   models_root, training_reports_root, legacy_v1=legacy_v1)
    ...
    if args.mode == "compare":
        run_compare(models_root, training_reports_root)

    passed = True
    if args.mode in ("validate", "all"):
        if legacy_v1:
            passed = run_validation(model_matrix_root, label_policy_root)
        else:
            passed = run_validation_v2(model_matrix_root, label_policy_root)

    print(f"\nOutput paths ({'v1 legacy' if legacy_v1 else 'v2 active'}):")
    print(f"  shared labels:     {label_policy_symbol_root(label_policy_root)}")
